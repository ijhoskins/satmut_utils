...
from scripts.run_read_inducer import workflow as read_inducer_workflow
...
HGVS_AA_FORMAT = "p.{}{}{}"
NNK_UNEXPECTED_WOBBLE_BPS = {"A", "C"}


class ErrorCorrectionDataGenerator(object):
    """Generates simulated data for training error correction models. Uses analysis.VariantCaller output."""

    DEFAULT_NVARS = None
    DEFAULT_CONSERVATIVE_MNPS = False

    def __init__(self, negative_summary, mutant_summary, negative_bam, ref, gff, primers=None, outdir=".",
                 output_prefix=None, single_end=False, nvars=DEFAULT_NVARS, realign=False, filter_edited=True,
        ...
        :param str outdir: Optional output directory to store generated VCFs and induced FASTQs, BAM
        ...
        :param int | None nvars: number of variants requested; set to None for equivalent nvars to the endogenous
        :param bool realign: realign the induced FASTQs? Needed for visualization of the induced BAM.
        :param bool filter_edited: filter the induced BAM for those reads/read pairs that were induced? Default True.
        ...
        self.filter_induced = filter_edited
        ...
        self.vg = vg.VariantGenerator(gff=gff, ref=ref, haplotypes=haplotypes, haplotype_len=haplotype_len,
    ...
    def get_mutant_caf_estimates(self):
        ...
        # We don't want the sample's background SNPs to inflate the estimates; filter out variants with high CAF
    ...
    def workflow(self, trx_id=vg.VariantGenerator.DEFAULT_TRANSCRIPT, out_vcf=None,
                 var_type=vg.VariantGenerator.DEFAULT_VAR_TYPE,
        ...
        _logger.info("Inducing variants into the negative control background.")
        outbam, r1_fastq, r2_fastq = read_inducer_workflow(
            am=self.negative_bam, variants=vp.out_vcf, reference=self.ref, primers=self.primers,
            output_dir=self.outdir, output_prefix=self.output_prefix,
            single_end=self.single_end, realign=self.realign,
            filter_induced=self.filter_induced)
